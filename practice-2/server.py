...
def identification(ip):
    ...
    conn.send(f"Идентификация прошла, {name}!".encode())
...
while True:
    if port > max_port:
        write_log(logging.error, "Остановка программы: не найдены порты!")

    try:
        sock.bind(('', port))
        write_log(logging.info, f"Связка с портом {port}")
        break
    except OSError:
        port += 1
